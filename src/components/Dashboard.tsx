...
export default function Dashboard({ onEditBadge }: { onEditBadge?: (badge: BadgeConfig) => void }) {
  ...
  const handleImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportStatus({ type: 'success', msg: `Imported ${count} badge${count !== 1 ? 's' : ''}` });
      await refresh();
    } catch (err) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button className="btn btn-sm btn-outline" onClick={handleExport} disabled={badges.length === 0}>
            Export
          </button>
          {!confirmClear ? (
            <button className="btn btn-sm btn-ghost text-error" onClick={() => setConfirmClear(true)} disabled={badges.length === 0}>
              Clear All
            </button>
          ) : (
            <div className="flex items-center gap-1">
              <span className="text-xs text-error">Sure?</span>
              <button className="btn btn-sm btn-error" onClick={handleClearAll}>Yes</button>
              <button className="btn btn-sm btn-ghost" onClick={() => setConfirmClear(false)}>No</button>
            </div>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-base-content/60 mb-6">Create badges in the Live Studio or import a snapshot.</p>
          <a href="/builder" className="btn btn-primary">Go to Live Studio</a>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {badges.map((badge) => (
